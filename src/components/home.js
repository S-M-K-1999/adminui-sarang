    ...
    <div className="container">
      ...
      <div className="input-group" style={{ marginTop: "100px" }}>
        <input
          ...
          onChange={(e)=>{setSearchvalue(e.target.value);setCurrpage(1)}}
        />
      </div>
          ...
          <MDBCol size="12">
            <MDBTable>
              <MDBTableHead dark>
                <tr>
                  <th scope="col">
                    <input type="checkbox" name="all" onChange={handleCheck}></input>
                  </th>
              ...
              {users && users.length === 0 ? (
                ...
                else if(ele.name.toLowerCase().includes(searchValue) ||
                ...
                .map((ele) => (
                  <MDBTableBody className={ele.isChecked ? 'userSelected':''}>
                    <tr>
                      <th scope="col">
                        <input type="checkbox" name={ele.name} onClick={handleCheck} checked={ele?.isChecked || false} ></input>
                      </th>
                      <td>
                        {edit.status === true  && edit.id===ele.id ?  <input  onChange={(e)=>ele['name']=e.target.value}></input>:ele.name }
                      </td>
                      <td>{edit.status === true  && edit.id===ele.id ?  <input onChange={(e)=>ele['email']=e.target.value}></input>:ele.email }</td>
                      <td>{edit.status === true  && edit.id===ele.id ?  <input onChange={(e)=>ele['role']=e.target.value}></input>:ele.role }</td>
                      <td>
                        {edit.status === true && edit.id === ele.id?<button
                          value={ele.id}
                          className="editbtn"
